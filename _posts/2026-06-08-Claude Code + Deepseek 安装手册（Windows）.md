...
1. **下载安装node.js**（官网链接：https://nodejs.org/zh-cn/download）

![](<images/Claude Code + Deepseek 安装手册（Windows）-image-12.png>)
...
![](<images/Claude Code + Deepseek 安装手册（Windows）-image-13.png>)
...
![](<images/Claude Code + Deepseek 安装手册（Windows）-image-11.png>)

下载安装包后进行安装，一直next就可以（安装路径可以自行修改，非必选项），安装成功后会打开如下应用界面。

![](<images/Claude Code + Deepseek 安装手册（Windows）-image-10.png>)
...
![](<images/Claude Code + Deepseek 安装手册（Windows）-image-14.png>)
...
![](<images/Claude Code + Deepseek 安装手册（Windows）-image.png>)
...
![](<images/Claude Code + Deepseek 安装手册（Windows）-image-1.png>)
...
![](<images/Claude Code + Deepseek 安装手册（Windows）-image-2.png>)

这样就能正常打开Claude了。

# 四、接入deepseek API

1. **打开deepseek API平台**（官方链接：https://platform.deepseek.com）

2. **创建一个API key**，然后记录保存key

> **PS：仅创建的时候可复制粘贴，之后无法查看和复制这个key，只能重新创建新的key才行了。最好不要把个人的key发给其他人，不然token计费走的是自己的账户。**

![](<images/Claude Code + Deepseek 安装手册（Windows）-image-3.png>)

* **在cc switch中配置API**

点击右上角的加号，添加供应商

![](<images/Claude Code + Deepseek 安装手册（Windows）-image-4.png>)

选择DeepSeek

![](<images/Claude Code + Deepseek 安装手册（Windows）-image-5.png>)

然后下滑，填写刚刚创建的key

![](<images/Claude Code + Deepseek 安装手册（Windows）-image-6.png>)
...
![](<images/Claude Code + Deepseek 安装手册（Windows）-image-7.png>)

然后点击“添加”，就可以保存deepseek模型接入开关了。

* **打开模型接入开关，开启Claude**

![](<images/Claude Code + Deepseek 安装手册（Windows）-image-8.png>)

在cmd中运行claude命令后，即可用deepseek接入Claude Code了。

> **PS：在使用期间，CC Swtich最好一直在后台运行，不要关闭。这个插件需要的后台资源很小，可以放心使用。**

![](<images/Claude Code + Deepseek 安装手册（Windows）-image-9.png>)

# 五、Vscode插件（可选，另一种使用方式）

大家也可以在vscode中下载Claude插件，也能自动被CC Swtich管理接入的模型。

![](<images/Claude Code + Deepseek 安装手册（Windows）-image-15.png>)

跟着前文的操作部署成功后，下载该插件后就可以使用了，使用效果如下图所示。

![](<images/Claude Code + Deepseek 安装手册（Windows）-image-16.png>)
